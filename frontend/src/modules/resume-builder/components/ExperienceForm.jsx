...
const ExperienceForm = ({ data: legacyData = [], onChange, sectionId }) => {
  ...
  const reduxItems = useSelector((state) =>
    selectSectionItems(state, sectionId),
  );
  ...
  useEffect(() => {
    if (
      activeItemId &&
      data.some((item) => item.id === activeItemId || item._id === activeItemId)
    ) {
  ...
  const updateItemHandler = (index, field, value) => {
    ...
    if (showModal && targetIndex === index && !isApplyingAi.current) {
      console.warn(
        `[Control Boundary] Blocked premature update for item ${index} while AI modal is open.`,
      );
      return;
    }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="border-b border-slate-100 pb-6">
        ...
        <p className="text-slate-500 mt-1">
          Detailed work history helps recruiters understand your background.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <SortableList
          items={data.map((item) => ({
            ...item,
            id: item.id || item._id || item.tempId,
          }))}
                  ...
                  <div className="p-6 bg-white animate-in slide-in-from-top-2 duration-200">
                    <div className="grid md:grid-cols-2 gap-5 mb-5">
                      <ValidatedInput
                        ...
                        value={item.position}
                        onChange={(e) =>
                          updateItemHandler(i, "position", e.target.value)
                        }
                      ...
                      <ValidatedInput
                        ...
                        value={item.company}
                        onChange={(e) =>
                          updateItemHandler(i, "company", e.target.value)
                        }
                      ...
                      <ValidatedInput
                        ...
                        value={item.location}
                        onChange={(e) =>
                          updateItemHandler(i, "location", e.target.value)
                        }
                    ...
                    <div className="grid md:grid-cols-2 gap-5 mb-5">
                      <ValidatedInput
                        ...
                        onChange={(e) =>
                          updateItemHandler(i, "start_date", e.target.value)
                        }
                        startIcon={Calendar}
                      />
                      <div className="space-y-2">
                        {!item.is_current ? (
                          <ValidatedInput
                            ...
                            value={item.end_date}
                            onChange={(e) =>
                              updateItemHandler(i, "end_date", e.target.value)
                            }
                            startIcon={Calendar}
                          />
                        ) : (
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              updated[i] = {
                                ...
                                end_date: isChecked
                                  ? null
                                  : updated[i].end_date, // Clear end date if current
                              };
                              onChange(updated);
                            }}
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <button
                          onClick={() =>
                            enhanceDescription(i, item.description)
                          }
                      ...
                      <div className="border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400 transition-all bg-white">
                        <RichTextEditor
                          ...
                          onChange={(value) =>
                            updateItemHandler(i, "description", value)
                          }
      ...
      <AIOptionsModal
        ...
        originalText={
          targetIndex !== null && data[targetIndex]
            ? data[targetIndex].description
            : ""
        }
        options={aiOptions || []}
