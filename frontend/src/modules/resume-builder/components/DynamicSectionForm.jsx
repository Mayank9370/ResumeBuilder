...
import {
  ...
  reorderItems,
  selectSectionItems,
} from "@/modules/resume-builder/state/resumeSlice";
...
const DynamicSectionForm = ({ section, onChange, onTitleChange }) => {
  ...
  const reduxItems = useSelector((state) =>
    selectSectionItems(state, section.id),
  );
  ...
  const data =
    reduxItems && reduxItems.length > 0 ? reduxItems : section.data || [];
  ...
  useEffect(() => {
    if (
      data.some(
        (item) =>
          typeof item === "object" && item !== null && !item.id && !item._id,
      )
    ) {
  ...
  const addNewItem = () => {
    if (["skills", "languages", "interests"].includes(type)) {
      ...
      if (section.id)
        dispatch(addItem({ sectionId: section.id, item: newItem }));
  ...
  const removeItem = (index) => {
    ...
    if (item && item.id && section.id)
      dispatch(deleteItem({ sectionId: section.id, itemId: item.id }));
  ...
  const enhanceDescription = async (index, key, text) => {
    ...
    try {
      const res = await axios.post(
        `${backendURL}/api/ai/enhance`,
        {
          text,
          type: section.type === "custom" ? "custom_section" : section.type,
        },
      ...
      if (res.data.success) {
        ...
        const rawOptions =
          res.data.options || res.data.enhancedText || res.data.text;
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex flex-wrap gap-2">
          {data.map((item, index) => (
            <div
              key={index}
              className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-2 rounded-xl shadow-sm hover:border-blue-300 transition-all"
            >
              <input
                type="text"
                value={item || ""}
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="border-b border-slate-100 pb-6">
        ...
        <p className="text-slate-500 mt-1">
          {type === "custom"
            ? "Define the content for this section."
            : `Add your ${type} details.`}
        </p>
      </div>
      ...
      <div className="space-y-6">
        <SortableList
          items={data.map((item) => ({
            ...item,
            id: item.id || item._id || item.tempId || `temp-${Math.random()}`,
          }))}
          onChange={handleDragChange}
          renderItem={(item, index) => {
            const titleField = Object.keys(config).find(
              (k) =>
                k.includes("title") ||
                k.includes("name") ||
            ...
            const subtitleField = Object.keys(config).find(
              (k) =>
                k.includes("subtitle") ||
                k.includes("role") ||
                k.includes("degree") ||
                ...
                <div className="border border-slate-200 rounded-xl bg-white overflow-hidden transition-all hover:border-blue-200 hover:shadow-sm">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${expandedId === item.id ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500"}`}
                      >
                        {index + 1}
                      </div>
                      <div>
                      ...
                      {Object.entries(config).map(([key, field]) => {
                        if (field.type === "checkbox") {
                          return (
                            <label
                              key={key}
                              className="flex items-center gap-2 text-sm font-medium text-slate-700 cursor-pointer w-fit select-none bg-white px-3 py-2 rounded-lg border border-slate-200"
                            >
                              <input
                                type="checkbox"
                                checked={item[key] || false}
                                onChange={(e) =>
                                  handleItemChange(index, key, e.target.checked)
                                }
                            ...
                            <div key={key}>
                              ...
                              <div className="border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400 transition-all bg-white">
                                <RichTextEditor
                                  ...
                                  onChange={(val) =>
                                    handleItemChange(index, key, val)
                                  }
