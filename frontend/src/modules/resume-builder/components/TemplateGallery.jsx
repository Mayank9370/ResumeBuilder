  ...
  const fetchGalleryData = async () => {
    ...
    try {
      ...
      if (
        ...
        featuresRes.value.data.features
      ) {
        const f = featuresRes.value.data.features.find(
          (f) => f.name === "RESUME_CREATE",
        );
      ...
      if (plansRes.status === "fulfilled" && plansRes.value.data.plans) {
        ...
        if (user && user.subscription?.planId) {
          const foundPlan = fetchedPlans.find(
            (p) => p.razorpayPlanId === user.subscription.planId,
          );
      ...
      if (templatesRes.status === "fulfilled" && templatesRes.value.data.data) {
        ...
        finalTemplates = finalTemplates.map((localT) => {
          const dbT = dbTemplates.find(
            (d) =>
              d.id === localT.id ||
              (d.name && d.name.toLowerCase() === localT.name.toLowerCase()),
          );
          if (dbT) {
            return {
              ...localT,
              ...
              image:
                dbT.imageUrl && dbT.imageUrl.length > 5
                  ? dbT.imageUrl
                  : dbT.image && dbT.image.length > 5
                    ? dbT.image
                    : localT.image,
              preview: dbT.description || localT.preview,
              tags: dbT.category
                ? [dbT.category, ...(dbT.tags || [])]
                : dbT.tags || [],
            };
          }
          return localT;
        ...
        const dbOnly = dbTemplates.filter((dbT) => {
          return !localTemplates.some(
            (localT) =>
              localT.id === dbT.id ||
              localT.name.toLowerCase() === (dbT.name || "").toLowerCase(),
          );
        });
        ...
        if (dbOnly.length > 0) {
          console.warn(
            "[TemplateGallery] Blocked %d unknown templates from DB (Strict Mode Violation):",
            dbOnly.length,
            dbOnly.map((t) => t.id),
          );
        }
        ...
        // Hoist the requested templates to the front
        const priorityNames = [
          "Pastel Dream",
          "Brutal Block",
          "The Legacy",
        ...
        const otherTemplates = mixedTemplates.filter(
          (t) => !priorityNames.includes(t.name)
        );

        priorityNames.forEach((name) => {
          const found = mixedTemplates.find((t) => t.name === name);
          if (found) priorityTemplates.push(found);
        });

        setTemplates([...priorityTemplates, ...otherTemplates]);
      } else {
        // Return local templates if API has no data but succeeds
        const mixedTemplates = mixTemplates(localTemplates);
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-pulse">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
            <div
              key={n}
              className="bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm h-96 flex flex-col"
            >
        ...
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-slate-300">
          ...
          <p className="text-slate-500 mt-2 max-w-sm mx-auto">
            We couldn't load any templates at the moment. Please try checking
            your connection or come back later.
          </p>
        </div>
      ) : (
